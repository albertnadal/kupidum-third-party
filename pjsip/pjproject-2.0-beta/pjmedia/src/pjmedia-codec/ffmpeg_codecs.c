/* $Id: ffmpeg_codecs.c 3922 2011-12-26 09:01:47Z bennylp $ */
/* 
...
#if YARN_HAS_VIDEO
	if ((c->encode2 && (desc->info.dir & PJMEDIA_DIR_ENCODING)) ||
...
#if YARN_HAS_VIDEO
	//AVCodecContext has no member named rc_lookahead anymore! To be removed from file in future versions.
	//ctx->rc_lookahead = 0;
        
        
    //Custom configurations for ffmpeg - by OTT Video Comms - To be further understood
    //Check AVCodec.h for options and AVCodecContext struct definition as well
    //Files to be checked are MpegVideo.c MpegVideo_enc.c etc...
    //Check http://linuxreviews.org/man/ffmpeg/ in order to identify possible options, then look into the code to find which flag needs to be turned on
    //Check http://ffmpeg.org/doxygen/trunk/structAVCodecContext.html
    //Check http://ffmpeg.org/doxygen/trunk/structAVCodec.html
    //Check http://ffmpeg.org/doxygen/trunk/structMpegEncContext.html
    //Check http://www.ece.cmu.edu/~ece796/h263.PDF
    //Check all options fur further tunning. Exemple umvplus configuration, etc...
        
    //GOP size configuration
    ctx->gop_size=30;  
    ctx->keyint_min=15; //(default)
        
    //EPZS motion search algorithm Configuration    
    //ctx->mv0_threshold=256;
        
    //LOOPFILTER Configuration    
    //do not use loop filter and obmc at the same time
    ctx->flags|=CODEC_FLAG_LOOP_FILTER;
        
    //OVERLAPPED MOTION COMPENSATION
    //ctx->flags|=CODEC_FLAG_OBMC;
        
    //Advanced AC intra prediction
    ctx->flags|=CODEC_FLAG_AC_PRED;
        
    //Error Concealment flags
    //ctx->error_concealment=?? FF_EC_DEBLOCK and FF_EC_GUESS_MVS
        
    //Advanced Motion Estimation Codec Flag    
    ctx->flags|=CODEC_FLAG_4MV;   //8x8 motion vectors (in addition to 16x16
    //    CODEC_FLAG_GMC
    //    CODEC_FLAG_MV0
    //    CODEC_FLAG_LOW_DELAY
    //    CODEC_FLAG_NORMALIZE_AQP
    //ctx->flags|=CODEC_FLAG_PART;   //Reorder information for improved error resiliency in network transmission
    //ctx->flags|=CODEC_FLAG_QPEL;   //Quarter Pel MC enabled

    // The CODEC_FLAG_H263P_UMV option was deprecated in the current and newest versions of the ffmpeg.
    // Despite that, this option was reactivated using the patch found at http://patches.libav.org/patch/7554/
    ctx->flags|=CODEC_FLAG_H263P_UMV;    //Allow the use of unlimited motion vector for prediction

    //ctx->flags|=CODEC_FLAG_LOW_DELAY; //seems to blow.
        
    //More options are possible, tbs in the source coded AVCodec.h and AVCodecContext struct
